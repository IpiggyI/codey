...
fn maintenance_status_exposes_structured_session_metrics() {
    ...
    assert_eq!(value["sessionStatus"], "ready");
...
fn startup_without_provider_rewrites_is_still_ready() {
    ...
    assert_eq!(summary.ghost_tasks_pruned, 0);
}

#[test]
fn session_index_cleanup_failure_marks_maintenance_error() {
    let cleanup = Err(anyhow::anyhow!("index cleanup failed"));
