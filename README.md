...
- 线路与模型：统一管理官方账号和第三方服务，支持多线路同时使用、按需停用线路、模型同步与手动维护、逐模型配置上下文窗口、自动压缩阈值和输出预留，兼容原有 1M 设置，支持全局默认模型和在同一任务中切换模型。关闭本地路由后，Codex 会直接使用当前线路，Codey 保留模型管理和旧任务的模型兼容。换用另一套配置模板后，不会把上一套地址的模型清单带到新地址。Codex 配置无法解析时会明确报错并拒绝启动，不会自动改写你的配置。
