...
- 线路与模型：请求由 Codex 直接发往你当前配置的线路，Codey 不再在本机做请求代理或线路绑定。控制台只读展示当前 Codex 线路（标识、地址、接口格式、是否官方账号鉴权），不再在控制台里新增、编辑或删除线路。模型目录从当前 Codex 线路同步，不沿用已保存线路上的旧清单。仍支持模型同步与手动维护、全局默认模型。使用 API Key 会话时，模型选择器仍保留 Fast 与思考强度。保存后这些选项会在之后打开模型选择器时更新；若仍未变化，按界面提示重启。换用另一套配置模板后，不会把上一套地址的模型清单带到新地址。Codex 配置无法解析时会明确报错并拒绝启动，不会自动改写你的配置。应用和恢复过程都不会改写你的 Codex 配置文件。生成的模型目录保存在 Codey 自己的数据目录，不会写入 Codex 主目录。逐模型上下文窗口、自动压缩阈值和输出预留在当前线路的模型列表中配置，兼容原有 1M 设置。
- 官方账号额度：本机有可用 ChatGPT 登录并开启额度显示时，在输入栏展示套餐窗口剩余百分比和重置时间，点开可查看 5 小时与 7 天窗口。额度跟随 ChatGPT 登录状态，不看当前所选模型或线路。没有 ChatGPT 登录时不显示额度。若当前服务是多人共用账号池，数字不一定代表本人消耗。
- 对话用量：当前对话有消耗后，输入栏会显示缓存命中、上下文占用和 Token 明细；中转线路同样显示这次对话的用量，不代表账号套餐。
- 会话增强：优化任务时间和运行状态展示，支持会话导入、导出、整段或指定轮次删除，以及最近备份恢复；启动时不会改写历史会话的线路归属；需要时可在控制台显式迁移历史内置路由会话；页面状态异常时会尝试重新同步。
- 历史线路兼容：线路改名、删除或停用后，旧任务可继续使用其他可确认的线路上的同一模型；无法确定线路时会提示重新选择。历史内置路由会话可在控制台显式迁移。
