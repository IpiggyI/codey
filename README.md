...
- 线路与模型：控制台只读展示当前 Codex 线路（标识、地址、接口格式、是否官方账号鉴权），不再在控制台里新增、编辑或删除线路。模型目录从当前 Codex 线路同步，不沿用已保存线路上的旧清单。仍支持模型同步与手动维护、全局默认模型。换用另一套配置模板后，不会把上一套地址的模型清单带到新地址。Codex 配置无法解析时会明确报错并拒绝启动，不会自动改写你的配置。逐模型上下文窗口、自动压缩阈值和输出预留在当前线路的模型列表中配置，兼容原有 1M 设置。
