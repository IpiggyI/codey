...
- 对话用量：当前对话有消耗后，输入栏会显示缓存命中、上下文占用和 Token 明细；打开已有用量的历史对话也会直接显示，无需再次发送消息。这是这次对话发生了什么，不是账号套餐还剩多少。你在本页发出一轮消息、并收到新用量之后，缓存命中数字会被剩余有效时间圈环绕：绿色表示还早，黄色表示快过期，红色表示马上过期；过期后圈消失，悬停可看到「已过期」。只是打开一条历史对话不会开始计时。账号额度百分比同样用圈环绕文字，以便和输入栏原来的上下文圈区分。有效时间默认 30 分钟，可在控制台按 1 到 180 分钟调整。这是本机估计，不是服务商返回的到期时刻。
